...
def filter_answers(answers_dset, min_occurence):
    ...
    print('Num of answers that appear >= %d times: %d' % (min_occurence, 
                                                          len(occurence)))
    return occurence
    ...
    ***answers_dset should be preprocessed***
    ...
    for ans_entry in answers_dset:
        ...
        for answer in answers:
            ...
            if answer_ not in ans2label:
                continue
            labels.append(ans2label[answer_])
            scores.append(answer['answer_confidence'])
...
def compute_target_mutant(answers_dset, ans2label, name, cache_root):
    """Augment answers_dset with soft score as label
    ...
    ***answers_dset should be preprocessed***
    ...
    for ans_entry in answers_dset:
        ...
        ### ORIG 
        orig_answers = ans_entry['orig_answers']
        orig_answer_count = {}
        ...
        for answer in orig_answers:
            answer_ = answer['answer']
            orig_answer_count[answer_] = orig_answer_count.get(answer_, 0) + 1

            if answer_ not in ans2label:
                continue
            orig_labels.append(ans2label[answer_])
            orig_scores.append(answer['answer_confidence'])

        orig_label_counts = {}
        for k, v in orig_answer_count.items():
            if k in ans2label:
                orig_label_counts[ans2label[k]] = v
        ...
        target.append({
            ...
            'labels': labels,
            'scores': scores, 
            'orig_question_id': ans_entry['orig_question_id'],
            'orig_image_id': ans_entry['orig_image_id'],
            'orig_label_counts': orig_label_counts,
...
def load_mutant():
    train_answer_file = "data/mutant_both_vqacp_v2/vqacp_v2_train_annotations.json"
    with open(train_answer_file) as f:
        train_answers = json.load(f)  # ['annotations']

    val_answer_file = "data/mutant_both_vqacp_v2/vqacp_v2_test_annotations.json"
    with open(val_answer_file) as f:
        val_answers = json.load(f)  # ['annotations']

    print("LOADED ANSWER FILES")

    ans2label = json.load(open("data/mutant_only_vqacp_v2/mutant_cp_merge_ans2label.json"))
    print("LOADED ANS2LABEL")

    compute_target(train_answers, ans2label, 'train', 
                          "data/mutant_both_vqacp_v2/cp-cache")
...
def main():
    ...
    parser.add_argument("dataset", choices=["cp_v2", "v2", "mutant"])
