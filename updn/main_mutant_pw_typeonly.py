...
from dataset_mutant_pw import Dictionary, VQAFeatureDataset, pwVQAFeatureDataset
import base_model_pw
from train_pw_typeonly import train
import utils
...
def parse_args():
    ...
    parser.add_argument(
        '--use_hdf5', action="store_true",
        help="use use_hdf5 for data loading")
    parser.add_argument(
    ...
    parser.add_argument('--tiny', action="store_true", help="tiny to debug")
...
def main():
    ...
    args = parse_args()
    TINY_TRAIN = args.tiny 
    dictionary = Dictionary.load_from_file('data/mutant_only_vqacp_v2/dictionary.pkl')
    cp = not args.nocp

    print("Building train dataset...")
    train_dset = pwVQAFeatureDataset('train', dictionary, 
                    dataroot='data/mutant_only_vqacp_v2/', 
                    cp=cp, cache_image_features=args.cache_features)
    print("Building test dataset...")
    eval_dset = VQAFeatureDataset('val', dictionary,  dataroot='data/mutant_only_vqacp_v2/', 
                    cp=cp, cache_image_features=args.cache_features)

    answer_voc_size = train_dset.num_ans_candidates

    # Compute the bias    # The bias here is just the expected score for each answer/question type
    ...
    model = getattr(base_model_pw, constructor)(train_dset, args.num_hid).cuda()
    model.w_emb.init_embedding('data/mutant_only_vqacp_v2/glove6b_init_300d.npy')
    ...
    train(model, train_loader, eval_loader, args.epochs, args.output, 
          args.eval_each_epoch, TINY_TRAIN)
