...
def _pw_create_entry(img_idx, question, answer, anno):
    answer.pop('image_id')
    ...
    entry = {
        ...
        'orig_question_id' : answer['orig_question_id'],
        'image_id'    : question['image_id'],
        'orig_image_id' : answer['orig_image_id'],
        'image_idx'   : img_idx,
        'question'    : question['question'],
        'orig_question' : question['orig_question'],
        'answer'      : answer 
    }
    return entry


def _pw_load_dataset(dataroot, name, img_id2val, cp=False, tiny=False):
    """Load entries
    ...
    """
    if cp:
        answer_path = os.path.join('data/mutant_only_vqacp_v2', 'cp-cache', '%s_target.pkl' % name)
        name = "train" if name == "train" else "test"
        question_path = os.path.join('data/mutant_only_vqacp_v2/', 'vqacp_v2_%s_questions.json' % name)
        anno_path = os.path.join('data/mutant_only_vqacp_v2/', 'vqacp_v2_%s_annotations.json' % name)
    ...
    for question, answer, anno in zip(questions, answers, annotations):
        ...
        entries.append(_pw_create_entry(img_idx, question, answer, anno))
...
class pwVQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', cp=False, use_hdf5=False, cache_image_features=False):
        super(pwVQAFeatureDataset, self).__init__()
        assert name in ['train', 'val']

        if cp:
            ans2label_path = os.path.join('./data/mutant_only_vqacp_v2', 
                                'cp-cache', 'trainval_ans2label.pkl')
            label2ans_path = os.path.join('./data/mutant_only_vqacp_v2', 
        ...
        self.entries = _pw_load_dataset(dataroot, name, imgid2idx, cp=cp)
    ...
    def tokenize(self, max_length=14):
        ...
        for entry in tqdm(self.entries, ncols=100, desc="tokenize"):
            ...
            orig_tokens = self.dictionary.tokenize(entry['orig_question'], False)
            orig_tokens = orig_tokens[:max_length]
            if len(orig_tokens) < max_length:
                # Note here we pad in front of the sentence
                padding = [self.dictionary.padding_idx] * (max_length - len(orig_tokens))
                orig_tokens = padding + orig_tokens
            utils.assert_eq(len(orig_tokens), max_length)
            entry['orig_q_token'] = orig_tokens
    ...
    def tensorize(self):
        for entry in tqdm(self.entries, ncols=100, desc="tensorize"):
            ...
            orig_question = torch.from_numpy(np.array(entry['orig_q_token']))
            entry['orig_q_token'] = orig_question

            answer = entry['answer']
            labels = np.array(answer['labels'])
            scores = np.array(answer['scores'], dtype=np.float32)
            orig_labels = np.array(answer['orig_labels'])
            orig_scores = np.array(answer['orig_scores'], dtype=np.float32)
            if len(labels):
            ...
            if len(orig_labels):
                ...
                orig_scores = torch.from_numpy(orig_scores)
                entry['answer']['orig_labels'] = orig_labels
                entry['answer']['orig_scores'] = orig_scores
            else:
                entry['answer']['orig_labels'] = None
                entry['answer']['orig_scores'] = None
    ...
    def __getitem__(self, index):
        ...
        if self.image_to_fe is not None:
            ...
            orig_img_id = entry["orig_image_id"]
            try: 
                img_id_str = '_'.join(img_id.split('_')[2:])
                orig_features = np.fromfile("data/trainval_features_mutant/" + str(img_id_str) + ".bin", np.float32)
            except:                           
                img_id_int = int(img_id.split('_')[-1])
                try:
                    orig_features = np.fromfile("data/trainval_features/" + str(img_id_int) + ".bin", np.float32)
                except:
                    orig_features = np.zeros((36, 2048), dtype=np.float32)
                    whoa_count += 1
            orig_features = torch.from_numpy(orig_features).view(36, 2048)


        question = entry['q_token']
        orig_question = entry['orig_q_token']
        answer = entry['answer']
        labels = answer['labels']
        scores = answer['scores']
        orig_labels = answer['orig_labels']
        orig_scores = answer['orig_scores']
        ...
        ### ORIG
        orig_target = torch.zeros(self.num_ans_candidates)
        if orig_labels is not None:
            orig_target.scatter_(0, orig_labels, orig_scores)
            try:
                _, max_idx = orig_scores.max(0)
            except:
                max_idx = 0

            orig_top_ans = orig_labels[max_idx]
            try:
                orig_top_ans_emb = self.ans_embed[int(orig_top_ans)]
            except:
                orig_top_ans_emb = self.ans_embed[int(orig_top_ans[0])]

        else:
            orig_top_ans_emb = self.ans_embed[0]


        if "bias" in entry:
            return features, question, typetarget, target, entry["bias"], torch.Tensor(answertypefeats), top_ans_emb, \
                   orig_features, orig_question, orig_target, orig_top_ans_emb
        else:
