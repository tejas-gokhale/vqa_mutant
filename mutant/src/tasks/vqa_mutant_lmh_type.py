...
from tasks.vqa_data_lmh_muttype import VQADataset, VQATorchDataset, VQAEvaluator
...
def get_bias(train_dset,eval_dset):
    ...
    answer_voc_size = train_dset.num_answers
    ...
    for ex in train_dset.data:
        ...
        if ans is not None:
            for label, score in ans.items():
                if label not in train_dset.ans2label:
                    continue
                label = train_dset.ans2label[label]
    ...
    # print("Bias",question_type_to_prob_array,flush=True)

    for ds in [train_dset,eval_dset]:
        for ex in tqdm(ds.data,"Adding Bias"):
            q_type = ex["question_type"]
...
class VQA:
    def __init__(self,folder="/",load=True):
        ...
        # Add bias to both train and valid
...
#         self.model = VQAModel(self.train_tuple.dataset.num_answers)
    ...
    def train(self, train_tuple, eval_tuple):
        ...
        for epoch in range(args.epochs):
            quesid2ans = {}
            for i, (ques_id, feats, boxes, sent, typetarget, target,bias, answertypefeats) in iter_wrapper(enumerate(loader)):

                self.model.train()
                self.optim.zero_grad()

                feats, boxes, target, typetarget, bias, answertypefeats = feats.cuda(), boxes.cuda(), target.cuda(), typetarget.cuda(), bias.cuda(), answertypefeats.cuda()
                hidden,logit,type_logit = self.model(feats, boxes, sent,ret_hid=True,typefeats=answertypefeats)
                ...
                # print(f"{type_logit_soft.shape},{typetarget.shape}",flush=True)

                logit = self.calculatelogits(logit,answertypefeats,"train")
                
                loss =  self.debias(hidden,logit,bias,target)
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        ...
        for i, datum_tuple in tqdm(enumerate(loader),ascii=True,desc="Evaluating"):
            ques_id, feats, boxes, sent, typed_target, target, bias, answertypefeats = datum_tuple   # Avoid seeing ground truth
            with torch.no_grad():
                feats, boxes = feats.cuda(), boxes.cuda()
                answertypefeats = answertypefeats.cuda()
                logit,typelogit = self.model(feats, boxes, sent,typefeats=answertypefeats)
                logit = self.sigmoid(logit)
                type_logit_soft = self.softmax(typelogit)
                logit = self.calculatelogits(logit,answertypefeats,"predict")
...
#               type_accuracy+= torch.mean(torch.eq((typelogit>0.5).int().cuda(),typed_target.cuda()).float().cuda()).cpu().item()
    ...
    def oracle_score(data_tuple):
        ...
        for i, datum_tuple in enumerate(loader):
            ques_id, feats, boxes, sent, typetarget, target, bias,answertypefeats = datum_tuple
