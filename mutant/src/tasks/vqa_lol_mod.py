...
from tasks.vqa_data_lol_mod import VQADataset, VQATorchDataset, VQAEvaluator
...
#         self.model = VQAModel(self.train_tuple.dataset.num_answers)
    ...
    def train(self, train_tuple, eval_tuple):
        ...
        for epoch in range(args.epochs):
            quesid2ans = {}
            for i, (ques_id, feats, boxes, sent, typetarget, yntypetarget, target) in iter_wrapper(enumerate(loader)):

                self.model.train()
                self.optim.zero_grad()

                feats, boxes, target, yntypetarget, typetarget = feats.cuda(), boxes.cuda(), target.cuda(), yntypetarget.cuda(), typetarget.cuda()
                ...
                loss = 0.9*loss + 0.05*loss_type + 0.05*loss_yn
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        ...
        for i, datum_tuple in tqdm(enumerate(loader),ascii=True,desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                logit,yn_type_logit,typelogit = self.model(feats, boxes, sent)
    ...
    def oracle_score(data_tuple):
        ...
        for i, datum_tuple in enumerate(loader):
            ques_id, feats, boxes, sent, typetarget, yntypetarget, target = datum_tuple
