...
from tasks.gqa_model_wsup_bins_3d_patches_embs_yn import GQAModel
from tasks.gqa_data_wsup_bins_3d_patches_embs_yn import GQADataset, GQATorchDataset, GQAEvaluator
...
def get_tuple(splits: str, bs:int, shuffle=False, drop_last=False, vqa=False) -> DataTuple:
    ...
    data_loader = DataLoader(
        tset, batch_size=bs,
#         shuffle=shuffle, 
        num_workers=24, sampler=sampler,
...
#             self.model.lxrt_encoder.multi_gpu()
    ...
    def train(self, train_tuple, eval_tuple):
        ...
        for epoch in range(args.epochs):
            quesid2ans = {}
            for i, (ques_id, feats, boxes, sent, img_feats, img_raw_feats, target, sp_target) in iter_wrapper(enumerate(loader)):

                self.model.train()
                
                feats, boxes, target = feats.to(self.device), boxes.to(self.device), target.to(self.device)
                img_feats = img_feats.to(self.device)
                img_raw_feats = img_raw_feats.to(self.device)
                sp_target = sp_target.to(self.device)
                
                with torch.cuda.amp.autocast(enabled=True):
                    logit, v_logits = self.model(feats, boxes, sent, img_feats, img_raw_feats)
                    assert logit.dim() == target.dim() == 2

                    sp_target = sp_target.view([sp_target.shape[0]*36*36*3])
                    v_logits = v_logits.view([v_logits.shape[0]*3*36*36,14])
...
#                 loss.backward()
    ...
    #                 self.optim.step()
                ...
                if self.valid_tuple is not None and (i%eval_itx==0 or i==total_len-1) and args.local_rank in [-1,0]:  # Do Validation
                    ...
                    valid_score, quesid2ans_eval = self.evaluate(eval_tuple)
                    if valid_score > best_valid:
                        best_valid = valid_score
                        eval_tuple[-1].dump_result(quesid2ans_eval,os.path.join(args.output, 'testdev_predict.json'))
                        self.save("BEST")
    ...
    def predict(self, eval_tuple: DataTuple, dump=None):
        ...
        for i, datum_tuple in tqdm(enumerate(loader),total=len(loader)):
            ques_id, feats, boxes, sent, img_feats, img_raw_feats = datum_tuple[:6]   # avoid handling target
            with torch.no_grad():
    ...
    def evaluate(self, eval_tuple: DataTuple, dump=None):
        ...
        return evaluator.evaluate(quesid2ans), quesid2ans

    @staticmethod
    def oracle_score(data_tuple):
        dset, loader, evaluator = data_tuple
        quesid2ans = {}
        for i, (ques_id, feats, boxes, sent, img_feats, img_raw_feats, target, sp_target) in tqdm(enumerate(loader),total=len(loader)):
    ...
    if args.test is not None:
        ...
        if 'submit' in args.test:
            gqa.predict(
                ...
                dump=os.path.join(args.output, args.test+'_submit_predict.json')
            )
        if 'testdev' in args.test:
            result, quesid2ans = gqa.evaluate(
                get_tuple('testdev_yesno', bs=args.batch_size,
                          shuffle=False, drop_last=False),
