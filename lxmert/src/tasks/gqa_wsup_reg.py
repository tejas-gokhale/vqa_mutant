...
from tasks.gqa_model_wsup_reg import GQAModel
from tasks.gqa_data_wsup_reg import GQADataset, GQATorchDataset, GQAEvaluator
...
#                 print(sp_target.shape,v_logits.shape)
                ...
                v_logits = v_logits.view([v_logits.shape[0],36*36,3])
#                 print(sp_target.shape,v_logits.shape)
...
#                 total_elem += corrs.shape[0]
                ...
                weak_loss = self.mse_loss(v_logits.float(),sp_target.float()).float() #* v_logits.size(1)
#                 print(weak_loss)
                

                
                loss = (loss.float() + weak_loss)/2
#                 print(loss)
...
#                     log_str += "\nSpatial Train Acc: %0.3f\n" % ((total_corr/total_elem)*100.)
                    log_str += "\nTrain Loss: %0.3f\n" % ((loss/total_bz))
