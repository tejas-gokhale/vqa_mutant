...
bin_walls = np.logspace(-100,0,15,base=1.08)
...
#         boxes = boxes.copy()
        ...
        try:
            spatial_datum = json.load(open("/data/data/lxmert_data/depth_bins/" + str(img_id) + ".15"))
            spatial_diff =  json.load(open("/data/data/lxmert_data/depth_diffs/" + str(img_id) + ".15"))
        except:
            spatial_datum = None
            spatial_diff = None
...
#             print(ques_id, feats, boxes, ques, target, torch.tensor(spatial_labels))
            return int(ques_id), torch.tensor(feats).float(), torch.tensor(boxes).float(), ques, torch.tensor(spatial_diff).float(), target, torch.tensor(spatial_labels)
        else:
#             print( ques_id, feats, boxes, ques)
            return int(ques_id), torch.tensor(feats).float(), torch.tensor(boxes).float(), ques, torch.tensor(spatial_diff).float()
