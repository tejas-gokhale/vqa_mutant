...
class SIRProcessor():
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, item) in enumerate(lines[0:200000]):
            ...
            labels = opmap[item["op"]]
            
            for text_b,label in zip([q1,q2],labels):
                text_a = ""
                examples.append(
...
def evaluate(args, model, tokenizer, prefix="",mode="dev",fname=""):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
    eval_task_names = ("mnli", "mnli-mm") if args.task_name == "mnli" else (args.task_name,)
    eval_outputs_dirs = (args.output_dir, args.output_dir + '-MM') if args.task_name == "mnli" else (args.output_dir,)

    results = {}
    evaluate = True if mode=="dev" else False
...
#             model = torch.nn.DataParallel(model)
        ...
        result = compute_metrics("qnli", preds, out_label_ids)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    processor = SIRProcessor()
    output_mode = "classification"
    # Load data features from cache or dataset file
    typet =  'dev' if evaluate else 'train'
    if test:
        typet=args.fname
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if evaluate and not test:
            ...
            examples = processor.get_test_examples(args.data_dir,args.fname)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result,preds,probs = evaluate(args, model, tokenizer, prefix=prefix)
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        result, predictions, probs = evaluate(args, model, tokenizer, mode="test",fname=args.fname)
            # Save results
        output_test_results_file = os.path.join(args.data_dir, args.fname+"test"+"_results.txt")
        with open(output_test_results_file, "w") as writer:
            for key in sorted(result.keys()):
                writer.write("{} = {}\n".format(key, str(result[key])))
        # Save predictions
        output_test_predictions_file = os.path.join(args.data_dir, args.fname+"test"+"_predictions.jsonl")
        with jsonlines.open(output_test_predictions_file, "w") as writer:
            examples = processor.get_test_examples(args.data_dir,args.fname)
            for idx,example in enumerate(examples):
                row={}
                row["qid"]=example.guid
                row["text_a"]=example.text_a
                row["text_b"]=example.text_b
                row["preds"]=int(predictions[idx])
                row["label"]=int(example.label)
                row["probs"]=probs[idx].tolist()
                writer.write(row)
    return results
